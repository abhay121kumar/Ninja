...
public:
    ...
    void insertWord(TriesNode *root, string word,int size,string newStr,int i)
    {
        // base condition
        if (word.size() == 0)
        {
        ...
        // recursive call
        insertWord(child, word.substr(1),size,newStr,i);
        if(word.size()==size){
            if(size==1){
                return;
            }
            word = newStr.substr(i+1);
            newStr = word;
            insertWord(child,word,size-1,newStr,i+1);
        }
    }
    void insertWord(string word)
    {
        ...
        insertWord(root, word,size,newStr,i);
    }
    // remove
...
int main()
{
    ...
    cout << t.searchTries("ab") << endl; // 1
    cout << t.searchTries("a") << endl; // 1
    cout << t.searchTries("def") << endl; // 1
    cout << t.searchTries("de") << endl; // 1
    // cout << t.searchTries("d") << endl; // 1
    // cout << t.searchTries("f") << endl; // 1
